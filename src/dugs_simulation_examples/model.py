...
from darts.engines import sim_params, property_evaluator_iface
from darts.physics.geothermal.physics import Geothermal
from darts.physics.geothermal.property_container import PropertyContainer
from darts.physics.properties.basic import ConstFunc
from darts.physics.properties.density import DensityBasic, Spivey2004
from darts.physics.properties.iapws.iapws_property import iapws_water_density_evaluator, temperature_region1_evaluator, \
    iapws_steam_density_evaluator, iapws_steam_viscosity_evaluator, iapws_total_enthalpy_evalutor, \
...
from dartsflash.eos_properties import EoSEnthalpy
from dartsflash.libflash import CubicEoS, AQEoS
from iapws import SeaWater
from iapws.iapws97 import _Bound_Ph, _Region1, _TSat_P, _Region4
...
class XarrayApi:
    ...
    def set_dynamic_grid_xarray(
        ...
        if self.nz is not None:
            ...
            self.xarray_dataset[dynamic_property_name][
            :, :, :, time_index
            ] = data_array
        else:
...
class Model(DartsModel):
    ...
    def __init__(self, set_nx, set_ny, set_nz, perms, poro,
                 set_dx, set_dy, set_dz, overburden, n_points=32, total_time = 30*365):
        ...
        # self.set_reservoir_serial_layers()
        self.set_reservoir()
        self.set_physics(n_points=n_points)
        self.set_sim_params(first_ts=1e-5, mult_ts=8, max_ts=365, runtime=self.total_time, tol_newton=1e-3,
                            tol_linear=1e-4,
    ...
    def set_initial_conditions(self, initial_values: dict = None, gradient: dict = None):
        self.physics.set_nonuniform_initial_conditions(self.reservoir.mesh, pressure_grad=107, temperature_grad=30)
    
    def set_simple_reservoir(self):
        base_thickness = 15
        underburden = self.overburden
        self.nz += (self.overburden + underburden)
        top_of_reservoir = 2000
        dz_list = []

        for i in range(self.overburden):
            dz_list.append(base_thickness)
            base_thickness *= 2

        overburden_dz_list = np.array(sorted(dz_list.copy(), reverse=True))
        underburden_dz_list = np.array(dz_list.copy())

        all_dz = np.concatenate([overburden_dz_list, [self.dz] * (self.nz - self.overburden*2), underburden_dz_list])

        # Calculate the depth of top confining layer
        total_thickness_overburden = sum(overburden_dz_list)
        z_start_adjusted = top_of_reservoir - total_thickness_overburden
        current_z = z_start_adjusted
        coordinates = []
        for dz in all_dz:
            midpoint = current_z + dz / 2
            coordinates.append(midpoint)
            current_z += dz

        depth = np.empty(self.nx * self.ny * self.nz)
        dz = np.empty(self.nx * self.ny * self.nz)

        for n in range(self.nz):
            depth[n * self.nx * self.ny:(n + 1) * self.nx * self.ny] = coordinates[n]
            dz[n * self.nx * self.ny:(n + 1) * self.nx * self.ny] = all_dz[n]
        ...
        self.xrdata = XarrayApi(nx=self.nx, ny=self.ny, nz=self.nz, total_time=self.total_time,
                                report_time=self.report_time, number_of_inj_wells=1, number_of_prd_wells=1,
                                last_time=True)
        self.xrdata.set_xarray_static('Perm', np.rot90(perm.reshape((self.nx, self.ny, self.nz),
                                                                    order='F')))
        self.xrdata.set_xarray_static('Poro', np.rot90(poro.reshape((self.nx, self.ny, self.nz),
                                                                    order='F')))
        # rock heat capacity and rock thermal conduction
        hcap = np.ones(self.nx * self.ny * self.nz)
        rcond = np.ones(self.nx * self.ny * self.nz)
        hcap[poro <= 1e-1] = 1500  # volumetric heat capacity: kJ/m3/K
        hcap[poro > 1e-1] = 2300
        ...
        self.reservoir = StructReservoir(self.timer, nx=self.nx, ny=self.ny, nz=self.nz, dx=self.dx, dy=self.dy,
                                         dz=dz,
                                         permx=perm, permy=perm, permz=0.1 * perm, poro=poro,
                                         depth=depth)
    ...
    def set_reservoir(self):
        ...
        if self.overburden == 0:
            ...
            set_dz_new = np.concatenate([overburden_dz, np.ones(self.nx * self.ny * self.nz) * self.dz, underburden_dz])
        ...
        depth = np.empty(self.nx * self.ny * self.nz)
        top = 1630
        unique_thickness = sorted(np.unique(self.dz))
        if len(unique_thickness) > 1:
            for k in range(self.nz):
                if k == 0:
                    d = top + unique_thickness[-1] / 2
                elif k == 1:
                    d = top + unique_thickness[-1] + unique_thickness[-2] / 2
                elif k == 2:
                    d = top + unique_thickness[-1] + unique_thickness[-2] + unique_thickness[-3] / 2
                elif k == 3:
                    d = top + unique_thickness[-1] + unique_thickness[-2] + unique_thickness[-3] + unique_thickness[
                        -4] / 2
                elif 4 <= k <= 19:
                    d = top + 225 + (2 * (k - 3) - 1) * unique_thickness[0] * 0.5
                elif k == 20:
                    d = top + 225 + 100 + unique_thickness[-4] / 2
                elif k == 21:
                    d = top + 225 + 100 + unique_thickness[-4] + unique_thickness[-3] / 2
                elif k == 22:
                    d = top + 225 + 100 + unique_thickness[-4] + unique_thickness[-3] + unique_thickness[-2] / 2
                elif k == 23:
                    d = top + 225 + 100 + unique_thickness[-4] + unique_thickness[-3] + unique_thickness[-2] + \
                        unique_thickness[-1] / 2

                depth[k * self.nx * self.ny:(k + 1) * self.nx * self.ny] = d
        else:
            for k in range(self.nz):
                d = top + (2 * k + 1) * unique_thickness[0] * 0.5
                depth[k * self.nx * self.ny:(k + 1) * self.nx * self.ny] = d
    ...
    def set_reservoir_serial_layers(self):
        underburden = self.overburden
        self.nz += (self.overburden + underburden)
        overburden_prop = np.ones(self.nx * self.ny * self.overburden) * 0.001
        underburden_prop = np.ones(self.nx * self.ny * underburden) * 0.001
    ...
    def set_wells(self):
        ...
        injection_well_x = int(1000 / self.dx)
        production_well_x = injection_well_x + int(1000 / self.dx)
        ...
        for k in range(start_index, end_index):
            self.reservoir.add_perforation("INJ1", cell_index=(injection_well_x, int(self.ny / 2), k),
                                           well_radius=0.16, verbose=True)

        self.reservoir.add_well("PRD1")
        for k in range(start_index, end_index):
            self.reservoir.add_perforation("PRD1", cell_index=(production_well_x, int(self.ny / 2), k),
                                           well_radius=0.16, verbose=True)
    ...
    def set_physics(self, n_points):
        ...
        property_container.density_ev = dict([('water', DensityBasic(compr=1e-5, dens0=1000/18.015)),
                                             ('steam', DensityBasic(compr=1e-5, dens0=0)),
                                            ])
        property_container.viscosity_ev = dict([('water', ConstFunc(0.79)),
                                                ('steam', ConstFunc(0.1))])
        self.physics = Geothermal(self.timer, n_points=n_points, min_p=100, max_p=500, min_e=1000, max_e=25000,
                                  cache=False, mass_rate=True)
...
class brine_density_evaluator(property_evaluator_iface):
    ...
    def evaluate(self, state):
        ...
        if region == 1:
            ...
            water_density = iapws_water_density_evaluator().evaluate(state) * 18.015 / 1000  # convert to g/cm3
        elif (region == 4):
            T = _TSat_P(P)
            if (T <= 623.15):
                ...
                raise NotImplementedError(
                    'Variables out of bound: p=' + str(P) + 'h=' + str(h) + ' region=' + str(region))
        elif (region == 2):
            water_density = 0
        else:
            raise NotImplementedError('Variables out of bound: p=' + str(P) + 'h=' + str(h) + ' region=' + str(region))
        T -= 273.15
        brine_density = water_density + self.brine_mass_fraction * (0.668 + 0.44 * self.brine_mass_fraction + 1e-6 *
                                                                    (300 * P - 2400 * P * self.brine_mass_fraction +
                                                                     T * (80 + 3 * T - 3300 * self.brine_mass_fraction -
                                                                          13 * P + 47 * P * self.brine_mass_fraction)))
...
class brine_viscosity_evaluator(property_evaluator_iface):
    ...
    def evaluate(self, state):
        ...
        if region == 1:
            ...
            T = temperature.evaluate(state)
        T -= 273.15
        brine_viscosity = 0.1 + 0.333 * self.brine_mass_fraction + (1.65 + 91.9 * self.brine_mass_fraction ** 3) * \
                          np.exp(-(0.42 * ((np.power(self.brine_mass_fraction, 0.8) - 0.17) ** 2) + 0.045)
                                 * np.power(T, 0.8))
